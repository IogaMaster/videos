...
import { CodeBlock, insert, lines, remove } from '@motion-canvas/2d/lib/components/CodeBlock';
...
export default makeScene2D(function*(view) {
    ...
    view.add(<Rect {...Container} ref={containerRef} fill="#00000000" gap={350} x={1500}>
    ...
    view.add(
        <Img src={pinImg} ref={pinRef} y={700} scale={1.5} />
    );
    ...
    containerRef().add(<CodeBlock code={`
    extra-experimental-features = nix-command flakes
    `} ref={nixconfRef} opacity={0} />)
    ...
    yield* containerRef().add(<CodeBlock language='nix' ref={flakeCodeRef} opacity={0} fontSize={38} code={`
{
    inputs = {
    ...
    yield flakeCodeRef().edit(1.2)`
{
    ...
    view.add(<Txt {...WhiteLabel} fontSize={58} y={500} opacity={0} ref={subtitleRef} > Also alias packages using a let binding.</Txt >)
    ...
    in
{
    ...
    yield* subtitleRef().text("I have linked some resources that go in depth about packaging applications with Nix.", 2.5, linear)

    yield* waitUntil("flake-utils first mention");
    yield*
        all(subtitleRef().text("", 2), flakeCodeRef().edit(1.2)`${remove(`outputs = { self, nixpkgs }: 
    let 
     pkgs = nixpkgs.legacyPackages.x86_64-linux;
    in
{
