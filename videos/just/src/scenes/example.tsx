...
import { Colors, WhiteLabel, BaseFont, setupView, Container } from "components";
...
export default makeScene2D(function* (view) {
  ...
  view.add(
    <Rect ref={logoContainerRef} scale={1.5} y={-1000} zIndex={-15}>
      <Img src={makeLogo} scale={0.4} />
      <Txt {...WhiteLabel} ref={logoTextRef} fontSize={58} y={245}>
        Make
      </Txt>
    </Rect>,
  );
  ...
  yield* waitFor(2);
  yield* all(errorsLogoRef().opacity(0, 0.1), rotateLogoRef().opacity(0, 0.2));
  ...
  view.add(
    <Img src={js} ref={jsLogoRef} opacity={0} scale={0.15} y={250} x={150} />,
  );
  yield* pythonLogoRef().x(-150, 0.4);
  yield* jsLogoRef().opacity(1, 0.15);
  yield* waitFor(2);
  yield* all(pythonLogoRef().opacity(0, 0.1), jsLogoRef().opacity(0, 0.2));
