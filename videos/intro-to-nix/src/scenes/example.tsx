...
export default makeScene2D(function* (view) {
  ...
  view.add(<Img ref={logoRef} src={nixLogo} y={-900} />);
  ...
  yield* all(langRef().remove(), packRef().remove(), operRef().remove());
  textLayoutRef().add(
    <Video src={createShellMp4} ref={createVideoRef} opacity={0} />,
  );
  yield* all(
    createVideoRef().opacity(1, 0.6, easeInOutCubic),
    createVideoRef().play(),
  );
  yield* delay(3, createVideoRef().opacity(0, 0.6, easeInOutCubic));
...
pkgs.mkShell {
  ...
  yield* delay(1.5, codeRef().selection(lines(0, 60), 0.6));
  ...
  yield* all(codeRef().remove(), shellRef().opacity(1, 0.6, easeInOutCubic));
  ...
  view.add(
    <Txt fill="#cdd6f4" fontFamily="JetBrains Mono" fontSize={120} zIndex={-1}>
      Thanks for watching!
    </Txt>,
  );
  yield* all(
    textLayoutRef().position(0, 0.6),
    textLayoutRef().size([0, 0], 0.6, easeInOutCubic, Vector2.arcLerp),
  );
