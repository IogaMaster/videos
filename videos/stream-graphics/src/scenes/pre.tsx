...
import { all, createRef, createSignal, delay, easeInOutCubic, waitFor } from '@motion-canvas/core';
...
export default makeScene2D(function*(view) {
    ...
    view.add(<Rect ref={logoContainerRef} scale={1} y={0} zIndex={-15}>
        <Circle ref={circleRef} width={() => radius()} height={() => radius()} fill={Colors.text} />
        <Img src={profile} scale={2} radius={360} />
        <Txt {...WhiteLabel} ref={streamTextRef} fontSize={58} rotation={1.1} y={350}>Stream starting soon...</Txt>
    </Rect>);

    yield* all(
        streamTextRef().scale(1.05, 1, easeInOutCubic).to(1, 1, easeInOutCubic),
        streamTextRef().rotation(-1.1, 1, easeInOutCubic).to(1.1, 1, easeInOutCubic),
