...
import { createEaseInElastic, easeInCubic, easeInOutCubic, linear } from '@motion-canvas/core/lib/tweening';
...
import installVideo from "../assets/install.mp4"; import lazygitStill from "../assets/lazygit.png";
...
export default makeScene2D(function*(view) {
    ...
    view.add(<Rect ref={logoContainerRef} scale={1.5} y={-900} zIndex={-15}>
        <Img src={git} scale={2} />
        <Txt {...WhiteLabel} ref={logoTextRef} fontSize={58} y={160}>Lazygit</Txt>
    </Rect>);
    ...
    view.add(<Img src={github} ref={webpageScrollRef} y={5600} opacity={0} scale={1.1}></Img>);
    yield* all(
    ...
    view.add(<Rect {...Container} ref={bigBoxRef} size={[1920 - 160, 1080 - 160]} zIndex={-1}><Video src={installVideo} ref={installVideoRef} /></Rect>);
    ...
    view.add(<Video src={lazygitVideo} ref={lazygitVideoRef} scale={0.85} opacity={0} />)
    yield* lazygitVideoRef().opacity(1, 0.3);

    yield* waitUntil("status");
    const highlightRef = createRef<Rect>();
    view.add(<Rect size={[525, 75]} ref={highlightRef} stroke={Colors.blue} lineWidth={8} x={-540} radius={20} zIndex={2} opacity={0} y={-405} />);
    yield* highlightRef().opacity(1, 0.4)


    yield* waitUntil("changes");
    yield* all(
        highlightRef().height(400, 1),
        highlightRef().y(-125, 1)
    )

    yield* waitUntil("branches");
    yield* all(
        highlightRef().height(90, 1),
        highlightRef().y(140, 1)
    )

    yield* waitUntil("commit history");
    yield* all(
        highlightRef().height(90, 1),
        highlightRef().y(245, 1)
    )

    yield* waitUntil("stashed files");
    yield* all(
        highlightRef().height(90, 1),
        highlightRef().y(360, 1)
    )
    ...
    view.add(
        <Txt {...WhiteLabel} scale={2.5} ref={txtRef} opacity={0} y={505}>a</Txt>
    )
